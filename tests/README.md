...
Use eval tests for stable AI/runtime safety claims that should remain readable
as product examples: destructive actions require confirmation, actor binding is
enforced, and stream replay preserves event history. The current deterministic
harness lives in `tests/evals/` and uses fake model/storage dependencies.
...
| `tests/test_support/storage_fakes.py` | Deterministic file-backed `CloudStorageClient` fake |
...
| `tests/evals/test_runtime_safety_evals.py` | Golden Nimbus runtime safety and replay evals |
...
| `src/nimbus_protocol/tests/test_protocol_properties.py` | Protocol event decoding and redacted error presentation invariants |
...
| `src/nimbus_runtime/tests/test_runtime_properties.py` | Runtime action, approval, and ACL-search safety invariants |
| `src/nimbus_runtime/tests/test_store_properties.py` | Durable worker-lease and artifact-store invariants |
| `src/nimbus_cli/tests/` | Python CLI onboarding, auth profiles, local session defaults, remote signing |
| `src/nimbus_slack/tests/` | Slack signature verification, dedupe, Events API handling, threaded posting |
...
| `src/aws_client_adapter/tests/test_service_adapter_properties.py` | Adapter mapping, sorting, delete-result, and error-detail invariants |

### OpenRouter Implementation

| File | Covers |
| --- | --- |
| `src/openrouter_ai_client_impl/tests/test_cli.py` | Legacy local REPL compatibility during the CLI split |
...
- Property/stateful invariants:
  `src/ai_client_api/tests/test_conversation_properties.py`,
  `src/nimbus_runtime/tests/test_runtime_properties.py`,
  `src/nimbus_runtime/tests/test_store_properties.py`
- BDD wrapper acceptance: `tests/bdd/test_wrapper_acceptance.py`
- Runtime eval harness: `tests/evals/nimbus_eval_harness.py`
