...
- **`AIClient`** — ABC with two abstract methods:
  ...
  - `stream_message(conversation, *, tools, max_steps=5, dry_run=False) -> AsyncIterator[AIStreamEvent]`
  - `ping() -> bool`
...
the workspace. Pass `interactive=True` to pick up interactive-mode tweaks.
Streaming is exposed through `AIClient.stream_message()` and consumed by
`nimbus_runtime`.
