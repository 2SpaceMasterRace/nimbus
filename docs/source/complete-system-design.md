...
```{admonition} Design Companion
:class: important

The canonical Nimbus product and system design lives in the repository root at
`SYSTEM_DESIGN.md`. This Sphinx page is a maintained companion for docs readers:
it summarizes contracts, state ownership, invariants, failure handling,
observability, scale triggers, and implementation order, but it must not drift
from or supersede the root design file.
```

Current implementation checkpoint:
: Nimbus ships with AWS S3 as the production storage backend. The code now has
  provider-neutral capability protocols, proof receipt artifacts with payload
  digests, protected roots, immutable generation manifests, generation diffs,
  drift verification, candidate cleanup plans, ordered storage stacks with
  restack conflicts, policy-patch learning proposals, replay traces, S3
  replica-lane healing proposals, provider-health probe artifacts, object
  provenance (`blame`), and S3-only migration decision packets. Non-S3 user
  workflows remain roadmap, but the extension boundary is real and tested
  through the fake-provider capability contract.

Nimbus is a Python 3.12+ workspace for provider-agnostic cloud storage and a
cloud-storage-aware AI runtime. The project deliberately avoids copying
large-company infrastructure when the workload does not require it. The goal is
stronger: build the smallest production-credible system whose primitives stay
correct under retries, crashes, duplicate delivery, and slow dependencies.
...
Goal
: Nimbus should be a technically deep system that still fits in one
  small-team-owned repository: provider-neutral storage, a concrete S3
  ...
  safely expose storage capabilities to chat-style interfaces through the
  Python CLI, Slack adapter, or signed HTTP boundary.
...
: The public surface is larger than function signatures. It includes
  ...
  `AIClient`, `Conversation`, `Tool`, `AIResponse`, `AIStreamEvent`,
  `NimbusEvent`, the Nimbus wrapper request and response schema, environment
  variables, CLI behavior, Slack callback behavior, persisted state shape, error
  messages, ordering guarantees, and documented operational assumptions.
...
: Large listings, large uploads/downloads, duplicate delivery, provider
  ...
  Slack retries, process restart, Postgres outage, stale schema, disk pressure,
  thread-pool saturation, stale credentials, and accidental horizontal scaling.
...
: AWS S3 through boto3, the external `cloud_storage_api` package, generated
  ...
  pydantic-ai, FastAPI/Starlette, Slack Events API, Render, Render Postgres,
  local JSON/SQLite fallback state under `AI_SESSION_DIR`, CircleCI, New Relic, Sentry,
...
: Unit tests protect contracts and small invariants. Integration tests protect
  ...
  wrapper flows. Deterministic evals protect golden runtime safety claims.
  End-to-end tests prove deployment shape. The next reliability frontier is
  deterministic simulation testing for duplicate delivery, crashes, ambiguous
  provider outcomes, replay, and overload.
...
```text
 ...
 nimbus_cli(local)           nimbus_cli(remote) / nimbus_slack / wrappers
        |                                      |
        v                                      v
  nimbus_runtime                       ai_server signed HTTP
        |                                      |
        +------------------+-------------------+
                           v
                    nimbus_runtime
 ...
 nimbus_protocol is the shared DTO/event/error vocabulary used by
 nimbus_cli, nimbus_slack, ai_server, and nimbus_runtime.
```
...
| `openrouter_ai_client_impl` | OpenRouter implementation, pydantic-ai loop, provider streaming, storage tool bindings | Provider-neutral contract definitions or CLI profile state |
| `nimbus_protocol` | Shared DTOs, stream events, errors, approvals, permission shapes | Runtime policy or transport rendering |
| `nimbus_runtime` | Session orchestration, replayable events, safe actions, confirmation flows, artifacts, telemetry | FastAPI request parsing or Slack-specific logic |
| `nimbus_cli` | Terminal onboarding, local in-process runtime profile, remote HTTP profile, auth storage | Runtime policy, provider SDK behavior, Slack handling |
| `nimbus_slack` | Slack signature verification, OAuth install, encrypted BYOK setup, Slack file diff/save commands, retry dedupe, event normalization, threaded reply posting | Runtime policy or provider calls |
...
| Artifacts | `nimbus_runtime.stores`, `nimbus_runtime.evidence` | SQLite metadata plus local content-addressed payload exports | Small evidence reports, local object-backed MVP | Immutable manifests; encrypted S3-backed large/cold payloads |
| Rate limits | `ai_server.router` | Process memory | One process | Shared backend only when multiple workers exist |
| OAuth/session tokens | `aws_client_service` | JSON files | Single-process service | Encrypted/expiring token store before multi-process or long-lived deployments |
...
Request latency, success rate, and failure rate are the baseline operational
signals. A serious design needs those plus enough context to debug saturation
...
The current test suite is broad: unit, integration, property, BDD, e2e, and
fuzz-smoke layers exist. The next step is to make reliability explicit.
...
- Keep root `SYSTEM_DESIGN.md` as the only canonical full system design
  document.
- Keep this page aligned as a Sphinx companion for docs readers.
...
When those questions are answered in code, docs, and tests, Nimbus stays a
compact example of how production systems are designed: contracts first, state
ownership explicit, failures expected, and scale introduced only where the
