...
This page documents the current Nimbus deployment shape and the operational
...
`/ready` and `scripts/render/start-slack.sh`; the Slack store applies its own
idempotent schema on startup/request while tenant-local Nimbus runtime state
uses the shared Postgres runtime backend.
...
| `AI_SERVER_SIGNED_REQUEST_MAX_BODY_BYTES` | `ai_server` | Maximum signed wrapper request body size |
...
| `NIMBUS_RAW_STORAGE_MUTATIONS_ENABLED`, `NIMBUS_RAW_STORAGE_ADMIN_KEY` | `aws_client_service` | Production gate and admin key for raw upload/delete routes |
| `NIMBUS_ENABLE_DEBUG_ROUTES` | `aws_client_service` | Enables deliberate debug routes such as `/sentry-debug` |
| `NIMBUS_S3_KMS_KEY_ID`, `NIMBUS_S3_SSE_KMS_REQUIRED` | `aws_client_impl` | SSE-KMS key and fail-closed S3 write enforcement |
...
| `NIMBUS_SLACK_PUBLIC_BASE_URL`, `NIMBUS_SLACK_STATE_SECRET`, `NIMBUS_SLACK_SECRET_KEY`, `NIMBUS_SLACK_SECRET_KEY_ID` | `nimbus_slack` | Public callback URL, OAuth state signing, and envelope-shaped encrypted secret storage |
| `NIMBUS_SLACK_SETUP_MAX_BODY_BYTES` | `nimbus_slack` | BYOK setup form body-size cap |
...
| `LOG_LEVEL` | telemetry | Structured log minimum level, default `INFO` |
...
or the runtime or Slack control-plane schema is stale.

## Telemetry and logs

Nimbus services call the shared `nimbus_runtime.observability` bootstrap during
startup. The naked system still works with stdout JSON logs only. When env vars
are present, the bootstrap adds:

- OpenTelemetry traces and metrics over OTLP/HTTP. `NEW_RELIC_LICENSE_KEY`
  automatically adds the `api-key` header for New Relic; generic OTLP backends
  can use `OTEL_EXPORTER_OTLP_HEADERS`.
- Sentry exception reporting through `SENTRY_DSN`.
- Pydantic Logfire export through `LOGFIRE_TOKEN`.
- FastAPI and HTTPX instrumentation so inbound requests and outbound Slack,
  OpenRouter, and Nimbus HTTP calls appear in traces.

New Relic is the primary telemetry destination. Sentry remains the exception
sink. Logfire is optional and useful for Python/Pydantic debugging. The runtime
still exposes in-process counters and histogram summaries so tests can assert
on observable behavior without depending on a vendor backend.

Keep the account-specific New Relic dashboard link in
`NIMBUS_TELEMETRY_DASHBOARD_URL` rather than committing it. The operator
dashboard should include request latency, success rate, failure rate,
AI/tool-call counts, and Slack reply outcomes from the metric names below.
...
| `nimbus_slack_turns_total` | counter | `kind`, `outcome` |
| `nimbus_slack_replies_total` | counter | `result`, `reason` |
...
| Missing `NIMBUS_S3_KMS_KEY_ID` in production | `/ready` fails and S3 writes fail closed. |
| Raw storage mutation in production without explicit gate | Upload/delete returns `403`; use runtime action ledger. |
| `/sentry-debug` without `NIMBUS_ENABLE_DEBUG_ROUTES=true` | Returns `404`. |
