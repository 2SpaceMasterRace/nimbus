...
the canonical system design for proving the future Nimbus session/action kernel
under hostile scheduling. Instead of sprinkling sleeps into concurrent tests, a
seeded scheduler controls time, task order, provider responses, retries,
crashes, duplicate messages, malformed records, and client reconnects.
...
The canonical design names the core pieces:
