...
This glossary is mostly distilled from the three agent-platform design passes:
{doc}`../nimbus/agent-platform-design`, {doc}`../nimbus/agent-platform-design-2`,
and {doc}`../nimbus/agent-platform-design-3`. Current code and package docs are
used as cross-checks for "what exists today"; the design trilogy is the primary
source for the platform vocabulary.
...
In the current codebase Nimbus is a Python workspace. In the design docs it is
also the name of the future agent-first operations platform.

See also: {doc}`../architecture-overview`, {doc}`../DESIGN`.

### Agent Operating Layer

The agent operating layer is the 2.0 product model: user intent becomes a
...
The design docs defer microservices until the package boundaries and access
...
The design docs explicitly avoid CRDTs for action correctness because
destructive state needs one authority.

### Event-Sourced Agent Session Runtime

The 3.0 design defines Nimbus as an event-sourced agent session runtime:
...
Today that role is mostly `nimbus_runtime`. The agent-platform design docs
argue that it should grow from turn orchestration into the authoritative
session/action engine.
...
The design docs distinguish operations from events. Operations are requests;
...
`ActionStatus` is the action state machine. The design docs use statuses such
...
MCP is the Model Context Protocol. The design docs treat it as a future
...
The 3.0 design also requires sequence numbers, schema versions, idempotency
...
The design docs call this out so durable logs do not become ambiguous transcript
dumps.
...
Operation priority ranks work during overload. The 3.0 design uses safety,
...
The design docs split time into wall clock, monotonic clock, and simulation
clock.
...
A runtime alert rule is the 2.0 design idea for structured, typed invariants
...
Postgres is the first serious shared durable backend in the design docs. It
...
payloads. The 3.0 design proposes event schema metadata: type, version, owner,
retention, privacy class, compatibility mode, and payload schema.
