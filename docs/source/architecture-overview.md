...
| AI | `ai_client_api` | `openrouter_ai_client_impl` | `nimbus_runtime`, `ai_server`, Nimbus CLI |
...
| `ai_client_api` | `AIClient`, `Conversation`, `Tool`, `AIResponse`, AI exceptions | Provider SDK imports |
| `openrouter_ai_client_impl` | OpenRouter + pydantic-ai implementation, CLI, storage tool bindings | The provider-agnostic contract |
| `nimbus_runtime` | Session orchestration, confirmation flows, attachment ingestion, telemetry | FastAPI request parsing |
