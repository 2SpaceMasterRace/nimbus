...
```shell
uv run python -c "import aws_client_impl, ai_client_api, nimbus_runtime, ai_server; print('ok')"
```
...
The CLI uses the `AIClient` contract through the OpenRouter implementation. It
can run without tools, or with storage tools pinned to one bucket and one safe
local directory.
