...
This guide gets a new contributor from a clean checkout to a running Nimbus
...
The storage FastAPI app lives in `aws_client_service.main:app`. It exposes:
...
`NimbusRuntime` in-process. Chat starts a fresh session by default; use
`resume` when you want the last session.
...
For a remote/self-hosted server, configure a separate remote profile. This is
for deployed API validation, not normal local CLI use:
...
uv run nimbus chat "hello through the deployed server" --profile dev-server
```
