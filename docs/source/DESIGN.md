...
Nimbus is a provider-neutral cloud-storage library and AI runtime, evolved in
two layered passes:

The first pass turned a local provider implementation into a deployable service
...
The second pass added the intelligent-application layer:
...
- Build a general MCP host/client/server stack today.
...
OpenTelemetry metrics/traces for production. New Relic is the default telemetry
...
| Use only the generated HTTP client in app code | Would expose HTTP details instead of preserving the in-process `CloudStorageClient` contract |
...
| Build MCP now | Useful conceptually, but too much host/client/server scope before the storage and AI verticals are stable |
...
- Request latency, success rate, and failure rate must be visible in deployment;
  the current code provides the application-level seams.
...
Storage-vertical requirements satisfied by this design:

- FastAPI service deployment unit.
- OAuth-capable service boundary.
- OpenAPI-generated client.
- Adapter restoring the original `CloudStorageClient` interface.
- `/health` endpoint.
- README and component documentation.
- CI-oriented test/lint/type workflow.

AI/runtime-vertical requirements addressed by this design:
