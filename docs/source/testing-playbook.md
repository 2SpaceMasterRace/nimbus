...
Use property tests for unit-level boundaries when the contract is an invariant
over many shapes rather than one important example. For example, the service
adapter properties generate response objects and error bodies to prove that
mapping, sorting, and domain exception classification stay stable across the
...
- “A stale action transition leaves action state and event history unchanged.”
- “Search returns only documents visible to the actor after tenant and ACL
  filtering.”
