...
Nimbus is the cloud-storage and AI platform in this repository. It combines a
provider-agnostic object-storage contract, an AWS S3 implementation, an HTTP
...
the Python `nimbus-cli`, the `nimbus-slack` adapter, or an HTTP wrapper.

The shipped storage backend today is AWS S3. The multi-cloud promise is
architectural: new storage providers slot in behind `CloudStorageClient` and
the existing service/adapter boundary without changing runtime or chat
semantics. The repository currently ships AWS S3 as the concrete backend.

The root `SYSTEM_DESIGN.md` file is the canonical product and system-design
source. Sphinx pages, including {doc}`complete-system-design`, are maintained
reader-friendly companions and should not supersede the root design file.

The docs are organized around the way contributors actually work: get the
project running locally, trace the architecture from the public boundary
inward, then use the reference pages when you need exact API, endpoint,
testing, or operations details.
...
Onboard and run Nimbus from the terminal:
...
- {doc}`cloud-storage/index` - the storage product manual: concepts, Python SDK
  ...
  semantics.
- {doc}`architecture-overview` - the system map: storage vertical, AI/runtime
  vertical, transport boundaries, state ownership, and failure modes.
- {doc}`complete-system-design` - a Sphinx companion to root `SYSTEM_DESIGN.md`
  with public contracts, invariants, bottlenecks, failure modes, and review
  questions.
...
- {doc}`nimbus/index` - focused guides for the AI service, signed wrapper
  contract, CLI, sessions, attachments, and verification.
...
- {doc}`security-hardening` - canonical side-effect authority, encryption
  guarantees, raw API posture, and local red-team tracks.
...
- {doc}`class/index` - design lineage: the course context, themes, and
  external materials that influenced the architectural choices.
