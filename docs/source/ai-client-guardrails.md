...
**Defense.** `send_message(..., max_steps=5)` and
`stream_message(..., max_steps=5)` cap the number of model turns. Hitting the
cap raises `AIStepBudgetExceededError` rather than silently continuing. The CLI
and runtime render this as a domain error while keeping the session usable.
...
**Threat.** A stray `openai.AuthenticationError` bubbles up into a channel
adapter, crashes the process, and drops conversation history.

**Defense.** `_call_model` catches `openai`'s exception hierarchy and
maps it to our own `AIAuthenticationError` / `AIRateLimitError` /
`AITimeoutError` / `AIProviderError`. Runtime and CLI adapters catch
...
`nimbus_runtime` persists provider/runtime events for replay; channel adapters
can render them to a terminal, Slack thread, or production telemetry sink.

## 10. API-key hygiene

Keys are read from `OPENROUTER_API_KEY` or the `nimbus_cli` secret store, never
stored in conversation JSON, and never printed by terminal or Slack adapters.
The workspace `.gitignore` excludes scratch credential directories so key files
that operators drop locally cannot be committed accidentally.
...
- **Streaming replay is local-first.** `NimbusRuntime.stream_chat_turn()` records
  replayable events now. Multi-process replay should graduate from the local
  store to a shared backend when we run multiple writable runtime nodes.
