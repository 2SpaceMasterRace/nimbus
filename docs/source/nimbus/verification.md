...
Then configure a remote CLI profile against the local server:
...
uv run nimbus chat "hello through HTTP" --profile local-server
```
...
curl https://nimbus-slack-staging.onrender.com/health
```
...
| Local HTTP | Signed `/ai/chat/turn` contract and server wiring. |
