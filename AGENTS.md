...
- `src/cloud_storage_client_api/`: abstract `CloudStorageClient` contract, domain exceptions, and the dependency-injection factory.
...
- `main.py`: CLI and dependency-injection demo entry point.
...
- Preserve the factory pattern in `cloud_storage_client_api.factory`; implementations register themselves by import side effect.
...
- `cloud_storage_client_api` must not import implementation packages or `boto3`.
- `aws_client_impl` is the only package that may import `boto3`.
- `aws_client_service` must obtain a client through `get_client()` and must not instantiate `S3Client` directly.
...
- Preserve the dependency-injection registration contract unless the user explicitly asks to change it.
...
- change the public interface of `cloud_storage_client_api`
- add a new third-party dependency
- modify CI/CD configuration, release automation, or root tool configuration
- modify the dependency-injection registration contract or import-time registration behavior
