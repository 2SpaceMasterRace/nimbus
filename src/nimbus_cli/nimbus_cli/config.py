...
class NimbusProfile:
    ...
    def from_json(cls, data: object) -> NimbusProfile:
        ...
        profile = cls(
            ...
            aws_region=_optional_str(data, "aws_region"),
...
class NimbusConfig:
    ...
    def with_active_profile(self, name: str) -> NimbusConfig:
        """Return a config with ``name`` selected as the active profile."""
        if name not in self.profiles:
            msg = f"Nimbus profile {name!r} is not configured"
            raise KeyError(msg)
        return NimbusConfig(
            active_profile=name,
            profiles=dict(self.profiles),
