...
class NimbusCLI:
    ...
    def _send_user_turn(self, text: str) -> None:
        ...
        if response.cost_usd_estimate is not None:
            self._total_cost_usd += response.cost_usd_estimate
    ...
    def _cmd_cost(self, _arg: str) -> bool:
        ...
        if self._total_cost_usd > 0:
            cost_tail = f"  ≈ ${self._total_cost_usd:.4f} USD"
        else:
            cost_tail = (
        ...
        self._console.print(
            ...
            f"output={self._total_output_tokens} total={total}{cost_tail}"
        )
        return True
