...
class NimbusCLI:
    ...
    def _send_user_turn(self, text: str) -> None:
        ...
        # Render the assistant reply as Markdown inside a faint panel so it's
        # visually distinct from tool-call chatter above it. Empty reply is
        # common when the model finishes after a tool call without commentary.
        ...
        if text_out:
            self._console.print(
                Panel(
                    ...
                    padding=(0, 1),
                    title=Text("nimbus", style=f"bold {_ASSISTANT_COLOR}"),
                    title_align="left",
                )
            )
        status = Text()
        status.append("  steps=", style=_INFO_COLOR)
        status.append(str(response.steps))
        status.append("  tokens=", style=_INFO_COLOR)
        status.append(str(response.tokens.total))
        status.append("  model=", style=_INFO_COLOR)
        status.append(response.model)
        if response.fallback_used:
            status.append("  (fallback)", style=_FALLBACK_COLOR)
    ...
    def _on_event(self, event: AgentEvent) -> None:
        ...
        if event.kind == "tool_call_started":
            ...
            if success:
                ...
                self._console.print(Text.assemble(marker, f"{name}: ok"))
            else:
