...
def test_ping_command_reports_reachable(tmp_path: Path) -> None:
    ...
    assert h.cli._handle_slash("/ping") is True  # noqa: SLF001
...
def test_status_command_shows_model_session_and_tokens(tmp_path: Path) -> None:
    ...
    h = _harness(tmp_path)
    h.cli._total_input_tokens = 10  # noqa: SLF001
    h.cli._total_output_tokens = 5  # noqa: SLF001
    assert h.cli._handle_slash("/status") is True  # noqa: SLF001
    output = h.output()
...
def test_status_command_shows_step_budget_and_flags(tmp_path: Path) -> None:
    ...
    h = _harness(tmp_path)
    h.cli._max_steps = 4  # noqa: SLF001
    h.cli._dry_run = True  # noqa: SLF001
    assert h.cli._handle_slash("/status") is True  # noqa: SLF001
    output = h.output()
    assert "4" in output
