...
def test_destructive_tools_declare_is_destructive(tmp_path: Path) -> None:
    """upload_file and delete_file should advertise that they mutate storage."""
    tools = _tools_by_name(FakeStorage(), tmp_path)
    assert tools["upload_file"].is_destructive is True
    assert tools["delete_file"].is_destructive is True
    assert tools["list_files"].is_destructive is False
    assert tools["download_file"].is_destructive is False
    assert tools["get_file_info"].is_destructive is False
