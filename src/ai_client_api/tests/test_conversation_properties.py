...
@given(st.booleans())
def test_as_int_treats_booleans_as_default(b: bool) -> None:  # noqa: FBT001
...
class ConversationMachine(RuleBasedStateMachine):
    ...
    def message_count_within_cap(self) -> None:
        assert len(self.conv._messages) <= self.conv.max_messages  # noqa: SLF001

    @invariant()
    def no_orphan_tool_message_at_head(self) -> None:
        if self.conv._messages:  # noqa: SLF001
            assert self.conv._messages[0].role is not Role.TOOL  # noqa: SLF001
    ...
    def len_counts_system_message(self) -> None:
        assert len(self.conv) == len(self.conv._messages) + 1  # noqa: SLF001
...
def test_conversation_stateful() -> None:
    ...
    """
    run_state_machine_as_test(ConversationMachine, settings=settings(max_examples=200))
