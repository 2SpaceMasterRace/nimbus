...
class Tool:
    ...
    ``is_destructive`` declares that the tool can mutate or remove user data.
    Runtimes should treat it as a hint to require explicit confirmation, log a
    structured destructive-action event, and refuse silent retries. The flag
    is a contract surface, not a substitute for in-handler validation: the
    handler must still enforce its own preconditions (such as ``confirm=true``
    on a delete tool). Defaults to ``False`` for read-only tools.
    """
...
class AIResponse:
    ...
    ``cost_usd_estimate`` is optional and provider-supplied: implementations
    that know the per-token price of ``model`` populate it; implementations
    that don't leave it ``None``. Treat it as a non-authoritative dashboard
    convenience, not billing truth.
    """
...
"""Provider-neutral streaming event kinds emitted by ``AIClient.stream_message``."""
...
class AIStreamEvent:
    ...
    ``sequence`` is monotonic within one ``stream_message`` call. Runtime
    adapters persist their own durable session sequence separately.
    """
