...
Pass `is_destructive=True` to mark a `Tool` whose handler can mutate or remove
user data. Runtimes use the flag for auditing — e.g. structured logs, telemetry
counters, and Sentry breadcrumbs at invocation time. The flag is a contract
hint, not a substitute for in-handler validation: a delete tool must still
enforce its own preconditions (such as `confirm=true`).

```python
delete_tool = Tool(
    name="delete_file",
    description="Delete a stored object. Requires confirm=true.",
