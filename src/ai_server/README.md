...
shape that Slack or future frontends can render.
...
| Idempotency | Cached response per `platform:workspace_id:idempotency_key` |
...
File-backed state is intentionally simple for the current deployment. The
...
| `NIMBUS_SAFE_ROOT` | No | current directory | Local filesystem sandbox for tools |
