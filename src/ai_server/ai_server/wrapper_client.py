...
def build_message_event_turn(
    *,
    ...
    if not isinstance(text, str):
        msg = "Slack message event must include string text"
        raise TypeError(msg)
    normalized_text = _strip_leading_app_mention(text).strip()
    if not normalized_text:
        msg = "Slack message event must include non-empty text"
        raise ValueError(msg)
    return {
