...
class StateReadResult:
    """Result of reading one request-state entry."""
...
class StateWriteResult:
    """Result of storing one request-state entry."""
...
def _session_dir() -> Path:
    """Return the request-state root directory base from the environment."""
...
def put_state_if_absent(
    ...
    *,
    ...
    with lock:
        now = time.time()
        cleaned_entries = _cleanup_namespace_locked(namespace, now=now)
        _, cleaned_target = _read_live_value(_entry_path(namespace, key), now=now)
        cleaned_entries += cleaned_target
        existing = _entry_path(namespace, key)
        if existing.is_file():
            return StateWriteResult(stored=False, cleaned_entries=cleaned_entries)
        _write_entry(namespace=namespace, key=key, value=value, expires_at=expires_at)
        return StateWriteResult(stored=True, cleaned_entries=cleaned_entries)
