...
class ConfirmationState(BaseModel):
    ...
    kind: Literal["delete_file", "copy_file", "move_file", "write_file"] = Field(
...
async def chat_turn(
    ...
    try:
        ...
        log.exception(
            "chat_turn_idempotency_cache_store_failed",
            request_id=request_id,
            conversation_id=conversation_id,
            idempotency_key=req.idempotency_key,
        )
    try:
        ...
        log.exception(
            "chat_turn_idempotency_claim_release_failed",
            request_id=request_id,
            conversation_id=conversation_id,
            idempotency_key=req.idempotency_key,
        )
