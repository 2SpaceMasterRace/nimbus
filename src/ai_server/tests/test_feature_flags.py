...
def test_launchdarkly_provider_returns_flag_value(
    ...
    class _FakeClient:
        def variation(self, key: str, ctx: object, default: bool) -> bool:
            return True

    monkeypatch.setattr(ldclient, "set_config", lambda *a, **kw: None)
    monkeypatch.setattr(ldclient, "get", lambda: _FakeClient())
...
def test_launchdarkly_provider_falls_back_on_sdk_exception(
    ...
    monkeypatch.setattr(ldclient, "set_config", lambda *a, **kw: None)
    monkeypatch.setattr(ldclient, "get", lambda: _BadClient())
    provider = LaunchDarklyFeatureFlagProvider("test-sdk-key")
