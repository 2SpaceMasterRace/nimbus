...
def _set_test_tool_container(monkeypatch: pytest.MonkeyPatch) -> None:
    """Pin both supported env names so ambient CI values cannot win precedence."""
    monkeypatch.setenv("NIMBUS_CONTAINER", "test-wrapper-bucket")
...
class TestChatTurnContract:
    ...
    def test_passes_full_storage_tool_surface_to_the_ai_client(
        self,
        client: TestClient,
        monkeypatch: pytest.MonkeyPatch,
        fake_client: FakeAIClient,
    ) -> None:
        # The wrapper exposes the full CRUD surface (including destructive
        # delete_file / move_file). Destructive execution is gated downstream
        # by the runtime's approval flow, not by withholding the tool.
        monkeypatch.setenv("AI_SERVER_SIGNING_SECRET", TEST_SIGNING_SECRET)
    ...
    def test_accepts_worst_case_identifier_lengths_without_session_overflow(
        ...
        # Exclude usage sidecar files (sha256-…_usage.json) added by the runtime.
        session_files = [
            f
            for f in (tmp_path / "sessions").glob("*.json")
            if not f.stem.endswith("_usage")
        ]
...
class TestChatTurnAuth:
    ...
    def test_rejects_oversized_nonce_before_persistence(
        self,
        client: TestClient,
        monkeypatch: pytest.MonkeyPatch,
    ) -> None:
        """Nonce shape is bounded before it can enter replay state."""
        monkeypatch.setenv("AI_SERVER_SIGNING_SECRET", TEST_SIGNING_SECRET)
        body = _turn_body(request_id="req-huge-nonce")
        encoded = _encoded_json_body(body)
        headers = _sign_headers(
            body=encoded,
            secret=TEST_SIGNING_SECRET,
        ...
        assert auth_mod._seen_nonces == {}

    def test_rejects_oversized_signed_body(
        self,
        client: TestClient,
        monkeypatch: pytest.MonkeyPatch,
    ) -> None:
        """Signed request bodies should have an auth-layer size ceiling."""
        monkeypatch.setenv("AI_SERVER_SIGNING_SECRET", TEST_SIGNING_SECRET)
        monkeypatch.setattr(auth_mod, "_SIGNED_REQUEST_MAX_BODY_BYTES", 8)
        encoded = _encoded_json_body(_turn_body(request_id="req-huge-body"))
        headers = _sign_headers(body=encoded, secret=TEST_SIGNING_SECRET)
        headers["Content-Type"] = "application/json"
...
class TestChatTurnIdempotency:
    ...
    def test_cache_store_failure_does_not_turn_completed_turn_into_500(
        self,
        client: TestClient,
        monkeypatch: pytest.MonkeyPatch,
        fake_client: FakeAIClient,
    ) -> None:
        """A post-runtime idempotency-cache failure must not hide success."""
        monkeypatch.setenv("AI_SERVER_SIGNING_SECRET", TEST_SIGNING_SECRET)
        body = _turn_body(request_id="req-cache-store-failure")

        def fail_put_state(*_args: object, **_kwargs: object) -> int:
            message = "state store unwritable"
            raise OSError(message)

        monkeypatch.setattr(router_mod, "put_state", fail_put_state)
    ...
    def test_in_flight_duplicate_claim_is_rejected_before_side_effects(
        ...
        conversation_id = router_mod._compose_conversation_id(req)
        cache_key = router_mod._idempotency_cache_key(
            req,
            conversation_id=conversation_id,
        )
        fingerprint = router_mod._idempotency_request_fingerprint(
            req,
            conversation_id=conversation_id,
        )

        try:
            router_mod._claim_in_flight_turn(
                cache_key,
                request_fingerprint=fingerprint,
            )
            with pytest.raises(router_mod._IdempotencyInFlightError):
                router_mod._claim_in_flight_turn(
                    cache_key,
                    request_fingerprint=fingerprint,
                )
        finally:
            router_mod._release_in_flight_turn(cache_key)
    ...
    def test_expired_cached_response_causes_fresh_ai_call(
        ...
        """After both caches expire, the same idempotency key triggers a fresh AI call."""
        monkeypatch.setenv("AI_SERVER_SIGNING_SECRET", TEST_SIGNING_SECRET)
        body = _turn_body(
            request_id="req-ttl",
        ...
        for entry in router_mod._idempotent_turns.values():
        ...
        router_mod.delete_state(
            router_mod._IDEMPOTENT_TURN_STATE_NAMESPACE,
            cache_key,
        )
...
class TestChatTurnToolWiring:
    def test_route_can_exercise_list_files_through_the_bound_tool_surface(
        ...
        _set_test_tool_container(monkeypatch)
        fake_storage_client.list_page_return = (
            [
    ...
    def test_model_delete_tool_call_becomes_runtime_confirmation(
        self,
        monkeypatch: pytest.MonkeyPatch,
        tmp_path: Path,
        fake_storage_client: FakeStorageClient,
    ) -> None:
        """A model-forged confirm=true must not bypass the action ledger."""
        monkeypatch.setenv("AI_SERVER_SIGNING_SECRET", TEST_SIGNING_SECRET)
        monkeypatch.setenv("AI_SESSION_DIR", str(tmp_path / "sessions"))
        monkeypatch.setenv("OPENROUTER_API_KEY", "fake-or-key-not-used")
        _set_test_tool_container(monkeypatch)
        ...
        app.dependency_overrides[router_mod.get_ai_client] = lambda: (
            _ToolCallingAIClient(
                ...
                tool_kwargs={"remote_path": "reports/old.csv", "confirm": True},
            )
        )
        app.dependency_overrides[router_mod.get_storage_client] = lambda: (
            fake_storage_client
        ...
        resp = _post_signed_turn(
            client,
            body=_turn_body(
                text="please remove the old report",
                idempotency_key="slack:T123TEAM:event:evt-model-delete",
                request_id="req-model-delete",
            ),
            secret=TEST_SIGNING_SECRET,
        )
        ...
        assert (
            payload["confirmation"]["expected_reply"] == "yes, delete reports/old.csv"
        )
        assert fake_storage_client.deletes == []

    @pytest.mark.parametrize(
        ("tool_name", "tool_kwargs", "expected_reply", "expected_call"),
        [
            (
                "copy_file",
                {
                    "source_path": "reports/source.csv",
                    "dest_path": "reports/copy.csv",
                    "overwrite": False,
                },
                "yes, copy reports/source.csv to reports/copy.csv",
                "copy",
            ),
            (
                "move_file",
                {
                    "source_path": "reports/source.csv",
                    "dest_path": "reports/moved.csv",
                    "overwrite": False,
                    "confirm": True,
                },
                "yes, move reports/source.csv to reports/moved.csv",
                "move",
            ),
            (
                "write_file",
                {
                    "remote_path": "reports/new.txt",
                ...
                (
                    "yes, write reports/new.txt "
                    f"sha256:{hashlib.sha256(b'approved body').hexdigest()}"
                ),
                "write",
            ),
        ],
    )
    def test_model_storage_mutations_execute_only_after_runtime_confirmation(
        self,
        monkeypatch: pytest.MonkeyPatch,
        tmp_path: Path,
        fake_storage_client: FakeStorageClient,
        ...
        """Copy, move, and overwrite-capable write go through action approval."""
        monkeypatch.setenv("AI_SERVER_SIGNING_SECRET", TEST_SIGNING_SECRET)
        monkeypatch.setenv("AI_SESSION_DIR", str(tmp_path / f"sessions-{tool_name}"))
        monkeypatch.setenv("OPENROUTER_API_KEY", "fake-or-key-not-used")
        _set_test_tool_container(monkeypatch)
        fake_storage_client.missing_objects.update(
            {"reports/copy.csv", "reports/moved.csv"}
        )

        app = FastAPI()
        app.include_router(router_mod.router, prefix="/ai")
        app.dependency_overrides[router_mod.get_ai_client] = lambda: (
            _ToolCallingAIClient(tool_name=tool_name, tool_kwargs=tool_kwargs)
        )
        app.dependency_overrides[router_mod.get_storage_client] = lambda: (
            fake_storage_client
        )
        ...
        first = _post_signed_turn(
            client,
            body=_turn_body(
                text=f"please {expected_call} this file",
                idempotency_key=f"slack:T123TEAM:event:evt-{tool_name}",
                request_id=f"req-{tool_name}-proposal",
            ),
            secret=TEST_SIGNING_SECRET,
        )
        second = _post_signed_turn(
            ...
            body=_turn_body(
                text=expected_reply,
                idempotency_key=f"slack:T123TEAM:event:evt-{tool_name}-confirm",
                request_id=f"req-{tool_name}-confirm",
            ),
            secret=TEST_SIGNING_SECRET,
        )
        ...
        assert first.json()["confirmation"]["expected_reply"] == expected_reply
        assert second.status_code == 200
        assert second.json()["outcome"] == "reply"
        assert second.json()["actions"][0]["status"] == "succeeded"
        if expected_call == "copy":
            assert fake_storage_client.copies == [
                {
                    "src_container": "test-wrapper-bucket",
                    "src_key": "reports/source.csv",
                    "dst_container": "test-wrapper-bucket",
                    "dst_key": "reports/copy.csv",
                }
            ]
            assert fake_storage_client.deletes == []
        elif expected_call == "move":
            assert fake_storage_client.copies[-1]["dst_key"] == "reports/moved.csv"
            assert fake_storage_client.deletes == [
                {
                    "container": "test-wrapper-bucket",
                    "object_name": "reports/source.csv",
                }
            ]
        else:
            assert fake_storage_client.uploads[-1] == {
                "container": "test-wrapper-bucket",
                "remote_path": "reports/new.txt",
                "body": b"approved body",
            }
...
class TestChatTurnRateLimiting:
    def test_same_user_id_in_different_workspaces_does_not_collide(
        ...
        router_mod._rate_buckets[exhausted_key] = router_mod._TokenBucket(
            tokens=0.0,
