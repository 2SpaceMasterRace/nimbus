...
The state store is a JSON-file-backed expiring key-value store used for pending
confirmations and request state:
...
The file-backed state model is production-credible for the current single-writer
deployment. Move to a shared backend before scaling to multiple writable
instances.
