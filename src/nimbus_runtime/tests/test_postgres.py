...
def test_migrate_creates_runtime_task_tables(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Runtime migrations should include durable task and lease schemas."""
    ...
    statements = "\n".join(query for query, _params in connection.statements)
    assert "CREATE TABLE IF NOT EXISTS tasks" in statements
    assert "CREATE INDEX IF NOT EXISTS tasks_by_tenant" in statements
    assert "CREATE INDEX IF NOT EXISTS tasks_by_session" in statements
    assert "ADD COLUMN IF NOT EXISTS policy_decision_json" in statements
    assert "CREATE TABLE IF NOT EXISTS worker_leases" in statements
    assert "CREATE INDEX IF NOT EXISTS worker_leases_by_expiry" in statements
    assert "CREATE TABLE IF NOT EXISTS search_documents" in statements
    assert "CREATE TABLE IF NOT EXISTS search_chunks" in statements
    assert "CREATE INDEX IF NOT EXISTS search_chunks_search_vector" in statements
...
def test_request_state_helpers(
    ...
    monkeypatch.setattr("nimbus_runtime.postgres.time.time", lambda: 100.0)
