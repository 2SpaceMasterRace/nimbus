...
The blocking correctness gaps in Priority 0 are now closed. The remaining work
is product-contract completion and production-readiness.

Main reasons:

- confirmation and richer outcome handling are not implemented in the wrapper
  route yet,
...
- `src/ai_server/tests/test_e2e.py` now covers the signed wrapper-facing route
  in addition to the legacy `/ai/chat` path
...
- the docs now point directly at the Python wrapper helper module and the signed
  smoke client
- the auth split remains explicit: legacy `X-API-Key` path vs signed wrapper path

Acceptance criteria:

- wrapper contract docs stay aligned with code and tests
- request and response examples are stable and beginner-readable
- auth split (`X-API-Key` legacy vs signed wrapper route) remains clear
...
### 7.1 Property and stateful testing

Carried from `plans.md`.

Acceptance criteria:

- add Hypothesis/property-style tests where invariants matter
- model failure/retry/idempotency properties explicitly
- consider stateful tests for storage and session behavior
...
- `src/ai_server/tests/test_wrapper_contract.py` now encodes the documented
...
- `src/ai_server/tests/test_wrapper_client.py` now verifies the Python reference
...
- future integrations such as GCP backend and self-hosted observability stack
...
AI_SERVER_API_KEY=<legacy-key-if-testing-legacy-route> \
