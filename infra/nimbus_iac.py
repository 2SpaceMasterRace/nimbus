#!/usr/bin/env python3
# ruff: noqa: BLE001,D101,D102,D103,D107,E501,EXE001,PLR0915,PLR2004,PTH123,S110,T201,TRY300
# This operator script intentionally uses print-style CLI output and a compact
# standalone client shape; keep it isolated from production runtime modules.
...
class FlyClient:
    ...
    def allocate_ip(self, app_name: str, ip_type: str, region: str) -> dict:
        return self._post(
            f"/v1/apps/{app_name}/ip_assignments",
            {
                "type": ip_type,
                "region": region,
            },
    ...
    def delete_machine(self, app_name: str, machine_id: str) -> None:
        try:
            self._post(
                f"/v1/apps/{app_name}/machines/{machine_id}/stop",
                {
                    "signal": "SIGTERM",
                    "timeout": "5s",
                },
    ...
    if "fly_ip_shared_v4" not in state["resources"]:
        ...
        rows.append(
            (
                "  (no-op)",
                "fly_ip.shared_v4",
                state["resources"]["fly_ip_shared_v4"].get("address", ""),
            )
        )
    ...
    if "fly_ip_v6" not in state["resources"]:
        ...
        rows.append(
            (
                "  (no-op)",
                "fly_ip.v6",
                state["resources"]["fly_ip_v6"].get("address", ""),
            )
        )
    ...
    if vol_key not in state["resources"]:
        rows.append(
            (
                green("+ create"),
                vol_key,
                f"{cfg['volume']['size_gb']} GB  region={cfg['app']['region']}",
            )
        )
    else:
    ...
    if "fly_machine.app" not in state["resources"]:
        ...
        rows.append(
            (
                "  (no-op)",
                "fly_machine.app",
                state["resources"]["fly_machine.app"].get("id", ""),
            )
        )
...
def cmd_apply(cfg: dict, state: dict, client: FlyClient) -> None:
    ...
    if not existing_vol:
        print(
            f"  {green('+ ' + vol_key)}  {vol_cfg['name']} ({vol_cfg['size_gb']} GB) …"
        )
        ...
        state["resources"][vol_key] = {
            "id": vol["id"],
            "name": vol["name"],
            "region": vol.get("region", cfg["app"]["region"]),
        }
        save_state(state)
        ...
        print(
            f"  {yellow('~ ' + vol_key)}  {existing_vol['id']} already exists — skipping."
        )
    ...
    if not existing_machine:
        ...
        print(
            f"  {yellow('~ fly_machine.app')}  {existing_machine['id']} already exists — skipping."
        )
...
def cmd_destroy(cfg: dict, state: dict, client: FlyClient) -> None:
    ...
    print(
        red(
            "  WARNING: This will permanently delete the machine, volume, IPs, and app."
        )
    )
...
def cmd_output(state: dict) -> None:
    ...
    if "fly_ip_shared_v4" in res:
        print(
            f"  {bold('ipv4')} = {cyan(res['fly_ip_shared_v4'].get('address', 'unknown'))}"
        )
    if "fly_ip_v6" in res:
